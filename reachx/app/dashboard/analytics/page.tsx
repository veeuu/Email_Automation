      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-6xl mx-auto px-8 py-8 space-y-7">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Emails Sent",   value: totalSent.toLocaleString(),    sub: openRate   ? openRate + "% open rate"    : "all campaigns",  color: "text-sky-600",     bar: "bg-sky-500",     barW: Math.min(totalSent, 100) },
              ...
              <div key={s.label} className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3 hover:shadow-sm transition-all">
                <div>
                    ...
                    {campaigns.map((c: Campaign) => {
                      ...
                      return (
                        <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-5 py-4 text-right font-semibold text-violet-600">{sent > 0 ? ((opened / sent) * 100).toFixed(1) + "%" : "—"}</td>
                          <td className="px-5 py-4 text-right font-semibold text-emerald-600">{sent > 0 ? ((clicked / sent) * 100).toFixed(1) + "%" : "—"}</td>
